...
/*   Updated: 2015/04/17 19:48:03 by exam             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				Warlock::forgetSpell(std::string const &s)
{
	_spells.erase(s);
}

void				Warlock::launchSpell(std::string const &s, ATarget &target)
{
	if (_spells[s] == NULL)
		return ;
	_spells[s]->launch(target);
}
